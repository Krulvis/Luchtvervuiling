...
luchtvervuiling.App = function () {
    ...
    $("#region-field").autocomplete({
        source: names,
        select: luchtvervuiling.instance.handleRegionUIClick
    });
    ...
    $('#overlay-button').on('click', function (event) {
        luchtvervuiling.instance.getOverlay();
    });

    $('#graph-button').on('click', function (event) {
        luchtvervuiling.instance.getGraph();
    });
 ...
 */
luchtvervuiling.App.prototype.getOverlay = function () {
    ...
    $.ajax({
        ...
        beforeSend: function () {
            ...
            downloadCSV.hide();
            error.hide();
            luchtvervuiling.instance.clearOverlays();
        },
        error: function (data) {
            button.html('error');
        ...
        if (data['error']) {
            ...
            button.html(luchtvervuiling.App.OVERLAY_BASE_BUTTON_NAME);
            var mapId = data['mapid'];
            var token = data['token'];
            $('#legend-max span').html(luchtvervuiling.App.format(data['max']));
            $('#legend-min span').html(luchtvervuiling.App.format(data['min']));
            downloadImg.show();
 ...
 */
luchtvervuiling.App.prototype.getGraph = function () {
    var startDate = $('#startDate').val();
    var endDate = $('#endDate').val();
    ...
    $.ajax({
        ...
        if (data['error']) {
            button.html(luchtvervuiling.App.GRAPH_BASE_BUTTON_NAME);
            error.show().html(data['error']);
        } else {
            button.html(luchtvervuiling.App.GRAPH_BASE_BUTTON_NAME);
 ...
 * @returns {boolean}
 */
luchtvervuiling.App.prototype.checkSelections = function (product, statistic, timestep) {
    var error = $('#error-message');
    error.hide();
 ...
 * @param ui
 */
luchtvervuiling.App.prototype.handleRegionUIClick = function (event, ui) {
    var countryName = ui.item.label;
    console.log('Clicked: ' + countryName);
    luchtvervuiling.instance.map.data.forEach(function (feature) {
        if (feature.getProperty('Country') === countryName) {
            luchtvervuiling.instance.map.data.revertStyle();
            luchtvervuiling.instance.selectedCountry = feature;
            luchtvervuiling.instance.map.data.overrideStyle(luchtvervuiling.instance.selectedCountry, luchtvervuiling.App.SELECTED_STYLE);
            return;
        }
    })
};
 ...
 */
luchtvervuiling.App.prototype.showChart = function () {
    $('.results').show();
 ...
 * @param event
 */
luchtvervuiling.App.prototype.switchStyle = function (event) {
     ...
     */
    this.clearOverlays();
    $('#overlay-button').html(luchtvervuiling.App.OVERLAY_BASE_BUTTON_NAME);
    $('#graph-button').html(luchtvervuiling.App.GRAPH_BASE_BUTTON_NAME);
    ...
    switch (this.selectionMethod) {
        ...
        case 'country':
            ...
            if (this.selectedCountry != null) {
                this.map.data.overrideStyle(this.selectedCountry, luchtvervuiling.App.SELECTED_STYLE);
            }
            break;
        case 'shapefile':
            overlayTab.removeClass('disabled');
 ...
 * @param event
 */
luchtvervuiling.App.prototype.switchOutput = function (event) {
 ...
 * @returns {string}
 */
luchtvervuiling.App.prototype.getProduct = function () {
 ...
 * @returns {string}
 */
luchtvervuiling.App.prototype.getTimestep = function () {
 ...
 * @returns {string}
 */
luchtvervuiling.App.prototype.getStatistic = function () {
 ...
 */
luchtvervuiling.App.prototype.getTarget = function () {
    switch (this.selectionMethod) {
        case "country":
 ...
 * @returns {*|string}
 */
luchtvervuiling.App.graphToCSV = function (dataTable) {
    var json = dataTable;
