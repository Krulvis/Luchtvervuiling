...
function loadOverlay(map) {
    ...
    $('.wmsLegend').html('<img src="' + legendURL + '"/>');

    // construct wms url
    var wmsParams = '?service=WMS&version=1.1.0&style=&width=256&height=256&srs=EPSG:900913&transparent=true&format=image/png&request=GetMap&layers=';

    if (time != "") {
        layerString += "_" + time;
    }
    var WMSURL = wms_root_url + wmsParams + layerString;
    console.log('Loading from: ' + WMSURL);
    loadWMS(map, WMSURL);
}
...
function loadWMS(map, baseURL, customParams) {
    ...
    console.log('Added overlaay');
    map.overlayMapTypes.setAt(0, overlayWMS, function () {
